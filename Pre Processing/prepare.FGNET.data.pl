...
my $data_path = "/home/ctext/Desktop/rico/AgeEstimation/fg-net/";
...
eval
{
    find( \&dir_list, $data_path . "images");
    ...
    foreach ( @content ) {
        ...
        if (index($parts[$length-1], '.') != -1) {
            my @fileparts = split('A', $parts[$length-1]);
            # Get the age from the filename
            my @ageparts = split('\.', $fileparts[1]);
            $ageparts[0] =~ /([0-9]+)/;
            my $age = $1;
    ...
    foreach my $category (sort(keys %all_data))
    {
        ...
        for (my $i = 0; $i < ceil($total_images*0.1); $i++) {
            ...
            my $length = scalar @parts;
            my $destination_path = $data_path . "fgnet_18_preprocessed/testing/" . $category;
            my $destination_file = $data_path . "fgnet_18_preprocessed/testing/" . $category . "/" . lc($parts[$length-1]);
        ...
        for (my $i = $total_copied; $i < (ceil($total_images*0.2) + $total_copied_testing); $i++) {
            my @parts = split('/', $images[$i]);
            my $length = scalar @parts;
            my $destination_path = $data_path . "fgnet_18_preprocessed/validation/" . $category;
            my $destination_file = $data_path . "fgnet_18_preprocessed/validation/" . $category . "/" . lc($parts[$length-1]);
            make_path($destination_path, {
                chmod => 0777,
            });
            copy $images[$i], $destination_file;
        ...
        for (my $i = $total_copied; $i < $total_images; $i++) {
            my @parts = split('/', $images[$i]);
            my $length = scalar @parts;
            my $destination_path = $data_path . "fgnet_18_preprocessed/training/" . $category;
            my $destination_file = $data_path . "fgnet_18_preprocessed/training/" . $category . "/" . lc($parts[$length-1]);
            make_path($destination_path, {
                chmod => 0777,
            });
            copy $images[$i], $destination_file;
